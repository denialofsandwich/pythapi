...
def api_mysql_connect():
    return MySQLdb.connect(
        host=config["core.mysql"]["hostname"],
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    return translation_dict["GENERAL_ERROR"][
                        config["core.general"]["default_language"]
                    ]
...
def try_convert_value(section, key, value, skel):
    ...
    if skel["type"] == bool and (section == "params" or section == "args"):
        if (
            (type(value) == bool and value)
            or type(value) != bool
            and (value.lower() == "true" or value == "1")
        ):
            value = True
        elif (
            (type(value) == bool and not value)
            or type(value) != bool
            and (value.lower() == "false" or value == "0")
        ):
            value = False
        else:
            raise WebRequestException(
            ...
            if desired_type == list:
                if type(value) == list:
                    if (
                        ...
                        "allow_duplicates" in skel
                        and skel["allow_duplicates"] == False
                    ...
                    if "childs" in skel:
                        for i, item in enumerate(value):
                            value[i] = try_convert_value(
                                section, "{}.{}".format(key, i), item, skel["childs"]
                            )
                ...
                if type(value) == dict:
                    if "childs" in skel:
                        for sub_key in skel["childs"].keys():
                            value[sub_key] = try_convert_value(
                                section,
                                "{}.{}".format(key, sub_key),
                                value.get(sub_key, None),
                                skel["childs"][sub_key],
                            )
    ...
    for section_name in config_defaults:
        ...
        for k in config_defaults[section_name]:
            ...
            try:
                ...
                print(
                    "ERROR in configuration. {}.{}: {} expected.".format(
                        section_name, k, type(v).__name__
                    )
                )
                sys.exit(1)
...
def api_action(plugin, pJSON):
    def ap_generator(f):
        ...
        if "path" in plugin.actions[-1]:
            plugin.actions[-1]["regex"] = (
                "^"
                + plugin.name
                + "/"
                + plugin.actions[-1]["path"].replace("*", "([^/]*)")
                + "$"
            )
