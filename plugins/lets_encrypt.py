...
plugin.info["f_name"] = {"EN": "Let's Encrypt"}
...
plugin.info["f_icon"] = {"EN": "vpn_lock"}
...
def _send_signed_request(url, payload):
    ...
    signature = i_exec_openssl(
        ["dgst", "-sha256", "-sign", keyfile_path],
...
def i_init_lets_encrypt():
    ...
    adtheaders = {
        "User-Agent": "pythapi-lets_encrypt/{}".format(plugin.version),
        "Accept-Language": "en",
    }
...
def i_setup_signatures():
    ...
    accountkey_json = json.dumps(
        jws_header["jwk"], sort_keys=True, separators=(",", ":")
    )
...
def i_domain_permission_validator(ruleset, rule_section, target_domain):
    ...
    for i_domain in domain_list:
        ...
        # Every subdomain
        elif i_domain_r[0] == "*" and ".".join(domain_r[1:]) == ".".join(
            i_domain_r[1:]
        ):
            return 1
    ...
    for rule in list(ruleset[section_name]):
        ...
        # reduce redundant * rules
            ...
            for rule2 in list(ruleset[section_name]):
                if (
                    rule != rule2
                    and rule2[-len(r_cut) :] == r_cut
                    and r_depth == len(rule2.split("."))
                ):
                    ruleset[section_name].remove(rule2)
...
def i_le_subset_intersection_handler(ruleset, subset):
    ...
    for rule in list(subset[section_name]):
        ...
        # On valid * rule
            ...
            rule[0:2] != "**" and "*" + rule[rule.find(".") :] in ruleset[section_name]
        ):
            _safe_append(return_subset, section_name, rule)
            continue
...
def i_get_direct_certificate(cert_id):
    ...
    if os.path.isfile(certfile_path):
        cert = i_exec_openssl(["x509", "-in", certfile_path, "-noout", "-text"]).decode(
            "utf8"
        )
    ...
    if write_through_cache_enabled:
        ...
        if "expires" in return_json:
            date = datetime.datetime.strptime(
                return_json["expires"], "%H:%M:%S %d.%m.%Y"
            )
            delta = date - datetime.datetime.now()
...
def it_complete_challenges(domain_list, order, order_location, **kwargs):
    ...
    for authz in order["authorizations"]:
        ...
        if resp.status_code != 200:
            raise ValueError(
                "Error fetching challenges: {0} {1}".format(
                    resp.status_code, authorization
                )
            )
        ...
        api_log().info(
            "Register this: {} at _acme-challenge.{}".format(keydigest64, domain)
        )
    ...
    if order["status"] == "pending":
        ...
        for v_handler in initverification_handler_list:
            ...
            if kwargs["_t_event"].is_set():
                try:
                    ...
                    pass  # If terminated due to a certificate deletion
                return
        ...
        for name, keydigest64 in token_dict.items():
            ...
            api_log().debug(
                "Pre-verification: Waiting for correct key of {}...".format(name)
            )
            while True:
                try:
        ...
        for authz in order["authorizations"]:
            ...
            challenge = [
                c for c in authorization["challenges"] if c["type"] == "dns-01"
            ][0]
            token = re.sub(r"[^A-Za-z0-9_\-]", "_", challenge["token"])
            keyauthorization = "{0}.{1}".format(token, thumbprint)
            ...
            code, result, headers = _send_signed_request(
                challenge["url"], {"keyAuthorization": keyauthorization}
            )
            if code != 200:
                raise ValueError(
            ...
            while True:
                try:
                    ...
                    raise ValueError(
                        "Error during challenge validation: {0} {1}".format(
                ...
                if challenge_status["status"] == "pending":
                    ...
                    api_log().info(
                        "ACME has verified challenge for domain: {0}".format(domain)
                    )
                    break
                else:
                    raise ValueError(
                        "Challenge for domain {0} did not pass: {1}".format(
        ...
        for v_handler in postverification_handler_list:
            cert_dict[cert_id]["status"] = (
                "running_post_verify_handler:" + v_handler.__name__
            )
            v_handler(domain_list, token_dict)
    ...
    while True:
        try:
            ...
            raise ValueError(
                "Error finalizing order: {0} {1}".format(
                    error.response.status_code, error.response.text()
                )
            )
        ...
        if finalize["status"] == "processing":
            ...
            cert_dict[cert_id]["status"] = "finalizing_error"
            raise ValueError(
                "Finalizing order {0} got errors: {1}".format(domain, finalize)
            )
    ...
    if resp.status_code != 200:
        cert_dict[cert_id]["status"] = "finalizing_error"
        raise ValueError(
            "Finalizing order {0} got errors: {1}".format(resp.status_code, resp.json())
        )
    ...
    api_log().info(
        "Certificate signed and chain received: {0}".format(finalize["certificate"])
    )
...
def e_renew_certificate(cert_id):
    ...
    # new order
    new_order = {
        "identifiers": [{"type": "dns", "value": domain} for domain in domain_list]
    }
    ...
    if code == 201:
        ...
        if not order["status"] in ["pending", "ready"]:
            raise ValueError(
                "Order status is not pending, we can't use it: {0}".format(order)
            )
    else:
        raise ValueError("Error getting new Order: {0} {1}".format(code, result))

    job.e_create_job(
        "le_request:{}".format(cert_id),
        it_complete_challenges,
        [domain_list, order, order_location],
        {},
        i_le_job_termination_handler,
    )
    ...
    for cert_path in cert_list:
        ...
        if not os.path.isfile(os.path.join(certfile_path)):
            ...
            try:
                ...
                log.error(
                    "Creation of certificate {} failed.".format(cert_id), exc_info=e
                )
            ...
            cert = i_exec_openssl(
                ["x509", "-in", certfile_path, "-noout", "-text"]
            ).decode("utf8")
            ...
            if delta.days < config["autorefresh_mindaysreaming"]:
                ...
                try:
                    ...
                    api_log().warning(
                        "Renewing already running for {}.".format(cert_id)
                    )
                except Exception as e:
                    log.error(
                        "Creation of certificate {} failed.".format(cert_id), exc_info=e
...
def it_add_certificate(domain_list, **kwargs):
    ...
    sysout = i_exec_openssl(
        ["genrsa", "-out", keyfile_path, str(config["rsa_keysize"])]
    )
    ...
    if write_through_cache_enabled:
        if cert_id in cert_dict:
            raise WebRequestException(
                400, "error", "LE_CERT_EXIST", {"cert_id": cert_id}
            )
...
def i_revoke_certificate(cert_id):
    ...
    certfile_path = os.path.join(
        config["base_key_directory"], "certs", cert_id, "certfile.pem"
    )
    ...
    code, result, headers = _send_signed_request(
        acme_config["revokeCert"], {"certificate": cert64}
    )
    if code != 200:
        raise ValueError(
            "Error while revoking the certificate: {0} {1}".format(code, result)
        )
...
def i_search_best_cert(domain_name):
    ...
    if not score_dict:
        raise WebRequestException(
            400, "error", "LE_CERT_NOT_FOUND", {"domain": domain_name}
        )
...
def i_rebuild_domain_links_p2(domain_list, certId_list):
    ...
    for domain, cert_id in zip(domain_list, certId_list):
        domainlink_path = os.path.join(
            domaindir_path, domain.replace("*", wc_replace_char)
        )
...
def install():
    ...
    auth.e_create_role(
        plugin.name + "_admin", {plugin.name + "_allowed_domains": ["**"]}
    )
    ...
    shutil.copyfile(
        "./lets_encrypt/openssl.cnf",
        os.path.join(config["base_key_directory"], "openssl.cnf"),
    )
    ...
    code, result, headers = _send_signed_request(
        acme_config["newAccount"], account_request
    )
    if code != 201:
...
def load():
    ...
    time_plugin.e_register_timed_static_event(
        "_lets_encrypt_job", et_check_certificates, [], enabled=1, repeat=1, **time_dict
    )
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "List certificates", "DE": "Liste Zertifikate auf"},
        "f_description": {
...
def list_certificates(reqHandler, p, args, body):
    ...
    for cert_id in list(cert_list):
        ...
        for domain in domain_list:
            if not auth.e_check_custom_permissions_of_current_user(
                plugin.name + "_allowed_domains", domain, i_domain_permission_validator
            ):
        ...
        "params": [
            {
                ...
                "f_name": {"EN": "Certificate ID", "DE": "Zertifikats ID"},
            }
        ],
        "f_name": {"EN": "Get certificate", "DE": "Zeige Zertifikat"},
        "f_description": {
...
@api_action(
    plugin,
    {
        ...
        "body": {
            "domains": {
                "type": list,
                "f_name": {"EN": "Domain list", "DE": "Domainliste"},
                "allow_empty": False,
        ...
        "f_name": {"EN": "Add new certificate", "DE": "Neues Zertifikat hinzufügen"},
        "f_description": {
...
def add_certificate(reqHandler, p, args, body):
    ...
    log.access(
        "{} certificate with domains {} and id {} created".format(
            api_environment_variables()["transaction_id"],
            domain_list,
            i_get_cert_id(domain_list),
        )
    )

    return {"cert_id": e_add_certificate(i_domains_to_punycode(domain_list))}
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "Delete certificate", "DE": "Zertifikat löschen"},
        "f_description": {
...
@api_action(
    plugin,
    {
        "path": "check",
        "method": "POST",
        "f_name": {"EN": "Check certificates", "DE": "Prüfe Zertifikate"},
        "f_description": {
...
@api_action(
    plugin,
    {
        ...
        "body": {
            ...
            "fingerprints": {
                "type": dict,
                "f_name": {"EN": "Fingerprints", "DE": "Fingerabdrücke"},
                "default": {},
            },
        },
        "f_name": {"EN": "Request Certificates", "DE": "Zertifikate anfordern"},
        "f_description": {
...
def request_certificates(reqHandler, p, args, body):
    ...
    log.access(
        "{} certificates with domains {} requested".format(
            api_environment_variables()["transaction_id"], domain_list
        )
    )
    ...
    for domain in domain_list:
        ...
        if not os.path.islink(cert_path):
            raise WebRequestException(
                400, "error", "LE_DOMAIN_NOT_FOUND", {"domain": domain}
            )
        ...
        if cert_fingerprint == body.get("fingerprints", {}).get(cert_id, {}).get(
            "cert", ""
        ):
        ...
        if key_fingerprint == body.get("fingerprints", {}).get(cert_id, {}).get(
            "key", ""
        ):
