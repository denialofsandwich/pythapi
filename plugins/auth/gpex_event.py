...
def unauthorized_error(error_code, error_name, error_message, remote_ip="N/A"):
    ...
    if auth_globals.bf_temporary_ban_enabled:
        if not remote_ip in auth_globals.bf_blacklist:
            ...
            auth_globals.bf_blacklist[remote_ip]["banned_until"] = (
                time.time() + ban_time
            )
...
def i_reset_ban_time(remote_ip="N/A"):
    ...
    if auth_globals.bf_temporary_ban_enabled:
        try:
            del auth_globals.bf_blacklist[remote_ip]
        except:
            pass
...
def i_log_access(message):
    if log.loglevel >= 5:
        log.access(
            "{} {}".format(api_environment_variables()["transaction_id"], message)
        )
...
def global_preexecution_hook(reqHandler, action):
    ...
    if auth_globals.bf_temporary_ban_enabled:
        if (
            remote_ip in auth_globals.bf_blacklist
            and auth_globals.bf_blacklist[remote_ip]["banned_until"] > time.time()
        ):
            remaining_time = math.ceil(
                auth_globals.bf_blacklist[remote_ip]["banned_until"] - time.time()
            )
    ...
    if auth_header is not None:
        ...
        if r_auth_header[0] == "Basic":
            ...
            if credentials[0] in auth_globals.users_dict:
                # User exists
                if (
                    e_hash_password(credentials[0], credentials[1])
                    == auth_globals.users_dict[credentials[0]]["h_password"]
                ):
                    # Passwort correct
                    ...
                    if not "users" in action:
                        ...
                        log.debug(
                            "You likely have a duplicated function name ({}).".format(
                                action["name"]
                            )
                        )
                        raise WebRequestException(
                            500, "error", "GENERAL_INTERNAL_SERVER_ERROR"
                        )

                    elif auth_globals.current_user in action["users"]:
                        # User permitted
                        i_log_access(
                            "authorized as {} via {}".format(
                                auth_globals.current_user, auth_globals.auth_type
                            )
                        )
                    ...
                    raise WebRequestException(
                        401, "unauthorized", "AUTH_PERMISSIONS_DENIED"
                    )

            unauthorized_error(
                401, "unauthorized", "AUTH_WRONG_PASSWORD_OR_USERNAME", remote_ip
            )
            ...
            if h_token in auth_globals.user_token_dict:
                ...
                auth_globals.current_user = auth_globals.user_token_dict[h_token][
                    "username"
                ]
                ...
                if not "token" in action:
                    ...
                    # Token permitted
                    i_log_access(
                        "authorized as {} via token {}".format(
                            auth_globals.current_user,
                            auth_globals.user_token_dict[h_token]["token_name"],
                        )
                    )
                    i_reset_ban_time(remote_ip)
                    return
    ...
    if action["name"] == "auth.create_session":
        ...
        if "username" in body and "password" in body:
            ...
            time.sleep(auth_globals.bf_basic_auth_delay)
            if (
                body["username"] in auth_globals.users_dict
                and e_hash_password(body["username"], body["password"])
                == auth_globals.users_dict[body["username"]]["h_password"]
            ):
    ...
    if session_id:
        if session_id in auth_globals.session_dict:
            ...
            if "last_csrf_token" in auth_globals.session_dict[session_id]:
                ...
                if (
                    csrf_token
                    != auth_globals.session_dict[session_id]["last_csrf_token"]
                ):
                    unauthorized_error(
                        401, "unauthorized", "AUTH_INVALID_CSRF_TOKEN", remote_ip
                    )
            ...
            auth_globals.current_user = auth_globals.session_dict[session_id][
                "username"
            ]
    ...
    if not "users" in action:
        log.error("Permission lookup table not found.")
        log.debug(
            "You likely have a duplicated function name ({}).".format(action["name"])
        )
        raise WebRequestException(500, "error", "GENERAL_INTERNAL_SERVER_ERROR")
