...
def install():
    ...
    manage_roles.e_create_role(
        "default", {"inherit": ["auth_default"], "permissions": []}
    )
    ...
    manage_users.e_create_user(
        "admin", "default", {"password": password, "ruleset": {"permissions": ["*"]}}
    )

    manage_users.e_create_user(
        "anonymous", "intern", {"password": e_generate_random_string(cookie_length)}
    )
