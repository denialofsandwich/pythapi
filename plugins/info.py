...
plugin.info["f_name"] = {"EN": "Information"}
...
plugin.info["f_icon"] = {"EN": "info"}

plugin.depends = [{"name": "auth", "required": False}]
...
def i_format_formatted_properties(return_json, property_name):
    ...
    try:
        translated_text = return_json[property_name][
        ...
        try:
            translated_text = return_json[property_name][
                api_config()["core.general"]["default_language"]
            ]
...
def i_format_action(action):
    ...
    if "auth" in api_plugins():
        ...
        if auth.e_check_custom_permissions_of_current_user(
...
def i_get_action_of_plugin(plugin_name, action_name):
    ...
    if "auth" in api_plugins():
        ...
        if api_config()[plugin.name][
            "hide_prohibited_actions"
        ] and not auth.e_check_custom_permissions_of_current_user(
            "permissions", action["permission"]
        ):
    ...
    for i_action in api_plugins()[plugin_name].actions:
        ...
        if "auth" in api_plugins():
            ...
            if api_config()[plugin.name][
                ...
                "permissions", i_action["permission"]
            ):
                continue
...
def install():
    ...
    if "auth" in api_plugins():
        ...
        auth.e_create_role("info_default", {"permissions": ["info.*"]})
...
def load():
    ...
    action_property_blacklist.extend(
        api_config()[plugin.name]["action_property_blacklist"]
    )
    plugin_property_blacklist.extend(
        api_config()[plugin.name]["plugin_property_blacklist"]
    )
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "List plugins", "DE": "Zeige alle Plugins"},
        "f_description": {
...
def list_plugins(reqHandler, p, args, body):
    ...
    if args["verbose"]:
        return {"data": i_list_plugins()}

    else:
        return {"data": list(api_plugins().keys())}
        ...
        "params": [
            {
                "name": "plugin_name",
                "type": str,
                "f_name": {"EN": "Plugin name", "DE": "Plugin Name"},
            }
        ],
        "f_name": {"EN": "Get plugin", "DE": "Zeige Plugin"},
        "f_description": {
    ...
    plugin,
    {
        ...
        "params": [
            ...
            },
            {
                "name": "action_name",
                "type": str,
                "f_name": {"EN": "Action name", "DE": "Action Name"},
            },
        ],
        "f_name": {"EN": "Get action of plugin", "DE": "Zeige Action eines Plugin"},
        "f_description": {
...
def get_action_of_plugin(reqHandler, p, args, body):
    return {"data": i_get_action_of_plugin(p[0], p[1])}
        ...
        "params": [
            {
                "name": "method",
                "type": str,
                "f_name": {"EN": "Method", "DE": "Methode"},
            },
            {"name": "path", "type": str, "f_name": {"EN": "Path", "DE": "Pfad"}},
        ],
        "f_name": {"EN": "Search action by path", "DE": "Suche Action mittels Pfad"},
        "f_description": {
...
def get_action_by_path(reqHandler, p, args, body):
    return {"data": i_get_action_by_path(p[0], p[1])}
...
@api_action(
    plugin,
    {
        ...
        "method": "GET",
        "permission": "get",
        "f_name": {"EN": "Get basic informations", "DE": "Zeige Basisinformationen"},
        "f_description": {
