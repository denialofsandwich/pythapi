...
plugin.info["f_name"] = {"EN": "Asynchronous Jobs", "DE": "Asynchrone Jobs"}
...
plugin.info["f_icon"] = {"EN": "cached"}

plugin.depends = []
plugin.config_defaults = {plugin.name: {"remove_on_termination": True}}

plugin.translation_dict = {
    "JOB_JOB_NOT_FOUND": {"EN": "Job not found.", "DE": "Job nicht gefunden."},
    "JOB_JOB_EXISTS": {"EN": "Job already exists.", "DE": "Job existiert bereits."},
}
...
class AsyncJob:
    def __init__(
        ...
        func_args=[],
        func_kwargs={},
        termination_handler=e_default_termination_handler,
    ):
...
def e_create_job(
    ...
    if job_name in job_dict and (
        job_dict[job_name].status != "done"
        and job_dict[job_name].status != "terminated"
    ):
        raise WebRequestException(400, "error", "JOB_JOB_EXISTS")

    job_dict[job_name] = AsyncJob(
        job_name, func, func_args, func_kwargs, termination_handler
    )
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "List jobs", "DE": "Zeige alle Jobs"},
        "f_description": {
...
def list_jobs(reqHandler, p, args, body):
    ...
    if args["verbose"]:
        return {"data": e_list_jobs()}

    else:
        return {"data": list(job_dict.keys())}
        ...
        "params": [
            {
                "name": "job_name",
                "type": str,
                "f_name": {"EN": "Job Name", "DE": "Job Name"},
            }
        ],
        "f_name": {"EN": "Get job", "DE": "Zeige Job"},
        "f_description": {
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "Terminate job", "DE": "Job beenden"},
        "f_description": {
