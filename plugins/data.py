...
plugin.info["f_name"] = {"EN": "Data storage", "DE": "Datenspeicher"}
...
def install():
    ...
    auth.e_create_role(
        ...
        {
            "permissions": ["data.*"],
            "data_permissions": ["/user/#username rw", "/global r"],
        },
    )
    ...
    auth.e_create_role(
        "data_admin", {"permissions": ["data.*"], "data_permissions": ["/ rw"]}
    )
...
@api_action(
    plugin,
    {
        ...
        "method": "GET",
        "permission": "read",
        "f_name": {"EN": "Read data", "DE": "Daten lesen"},
        "f_description": {
...
def read_data(reqHandler, p, args, body):
    ...
    if not auth.e_check_custom_permissions_of_current_user(
        plugin.name + "_permissions", p[0] + " r", i_data_permission_validator
    ):
        raise WebRequestException(401, "unauthorized", "AUTH_PERMISSIONS_DENIED")

    return {"data": e_read_data(p[0])}
...
@api_action(
    plugin,
    {
        ...
        "method": "POST",
        "permission": "write",
        "f_name": {"EN": "Write data", "DE": "Schreibe Daten"},
        "f_description": {
...
def write_data(reqHandler, p, args, body):
    ...
    auth = api_plugins()["auth"]
    if not auth.e_check_custom_permissions_of_current_user(
        plugin.name + "_permissions", p[0] + " w", i_data_permission_validator
    ):
        raise WebRequestException(401, "unauthorized", "AUTH_PERMISSIONS_DENIED")

    return {"affected_rows": e_write_data(p[0], body)}
...
@api_action(
    plugin,
    {
        ...
        "method": "DELETE",
        "permission": "delete",
        "f_name": {"EN": "Delete data", "DE": "Daten löschen"},
        "f_description": {
...
def delete_data(reqHandler, p, args, body):
    ...
    return {"affected_rows": e_delete_data(p[0])}
