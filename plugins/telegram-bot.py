...
plugin.info["f_name"] = {"EN": "Telegram-bot", "DE": "Telegram-Bot"}
...
plugin.info["f_icon"] = {"EN": "chat"}

plugin.depends = [{"name": "auth", "required": False}]

plugin.config_defaults = {
    plugin.name: {"bot_token": "no_string_given", "input_handler_enabled": True}
}
...
def i_get_db_channel(channel_name):
    ...
    sql = (
        ...
        SELECT * FROM """
        + db_prefix
        + """telegram_bot_channel WHERE channel_name = %s;
    """
    )
...
def i_get_local_channel(channel_name):
    ...
    return {"members": list(channel_dict[channel_name])}
        ...
        try:
            ...
            if dbc.rowcount == 0:
                raise WebRequestException(
                    400, "error", "TELEGRAM_CHAT_OR_CHANNEL_NOT_FOUND"
                )

        except MySQLdb.IntegrityError as e:
            api_log().error(
                "e_remove_reciever_from_channel: {}".format(api_tr("GENERAL_SQL_ERROR"))
            )
            raise WebRequestException(501, "error", "GENERAL_SQL_ERROR")
...
def e_send_image_to_channel(channel_name, image_path):
    ...
    for member in e_get_channel(channel_name)["members"]:
        image.seek(0)
        try:
            bot.send_photo(member, photo=image)
        except telegram.error.TimedOut:
            api_log().warning("e_send_text_to_channel: Timed out.")
            pass
...
def tc_start(bot, update):
    ...
    update.message.reply_text(
        "Hello. This is your chat_id: {}".format(str(update.message.chat_id))
    )
...
def load():
    ...
    try:
        bot = telegram.Bot(api_config()[plugin.name]["bot_token"])
    except:
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "List channel", "DE": "Channel auflisten"},
        "f_description": {
...
def list_channels(reqHandler, p, args, body):
    if args["verbose"]:
        return {"data": e_list_channels()}

    else:
        return {"data": list(channel_dict.keys())}
        ...
        "params": [
            {
                "name": "channel_name",
                "type": str,
                "f_name": {"EN": "Channel name", "DE": "Channel Name"},
            }
        ],
        "f_name": {"EN": "Get channel", "DE": "Zeige Channel"},
        "f_description": {
...
@api_action(
    plugin,
    {
        "path": "channel",
        "method": "GET",
        "f_name": {"EN": "Get own channel", "DE": "Zeige eigenen Channel"},
        "f_description": {
...
def get_personal_channel(reqHandler, p, args, body):
    ...
    return {"data": e_get_channel(current_user)}
    ...
    plugin,
    {
        ...
        "params": [
            ...
            },
            {
                "name": "username",
                "type": str,
                "f_name": {"EN": "Username", "DE": "Benutzername"},
            },
        ],
        "f_name": {
...
@api_action(
    plugin,
    {
        ...
        "body": {
            "message": {"type": str, "f_name": {"EN": "Message", "DE": "Nachricht"}}
        },
        "f_name": {"EN": "Send text to channel", "DE": "Sende Text an Channel"},
        "f_description": {
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "Send text", "DE": "Sende Text"},
        "f_description": {
...
@api_action(
    plugin,
    {
        ...
        "f_name": {"EN": "Send image to channel", "DE": "Sende Bild an Channel"},
        "f_description": {
...
@api_action(
    plugin,
    {
        "path": "send/image",
        "method": "POST",
        "f_name": {"EN": "Send image", "DE": "Sende Bild"},
        "f_description": {
